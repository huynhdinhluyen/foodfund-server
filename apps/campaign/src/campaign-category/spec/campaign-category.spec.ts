...
describe("CampaignCategoryService", () => {
    ...
    describe("validateCategoryExists", () => {
        ...
        describe("Normal values (Happy path)", () => {
            it("should not throw when category is active", async () => {
                // Arrange
                repository.isCategoryActive.mockResolvedValue(true)

                // Act & Assert
                await expect(
                    service.validateCategoryExists(categoryId),
                ).resolves.not.toThrow()
                expect(repository.isCategoryActive).toHaveBeenCalledWith(
                    categoryId,
                )
            })
        })
        ...
        describe("Abnormal values (Error cases)", () => {
            it("should throw BadRequestException when category is inactive", async () => {
                // Arrange
                repository.isCategoryActive.mockResolvedValue(false)
            ...
            it("should capture error and throw BadRequestException when repository fails", async () => {
                // Arrange
                const dbError = new Error("Database error")
                repository.isCategoryActive.mockRejectedValue(dbError)

                // Act & Assert
                await expect(
                    service.validateCategoryExists(categoryId),
                ).rejects.toThrow(
                    new BadRequestException("Invalid category ID provided"),
                )
                expect(sentryService.captureError).toHaveBeenCalledWith(
                    dbError,
                    expect.objectContaining({
