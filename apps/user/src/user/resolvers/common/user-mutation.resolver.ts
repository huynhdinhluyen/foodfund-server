...
export class UserMutationResolver {
    ...
    async updateMyProfile(
        ...
        const currentUser = await this.userQueryService.findUserByCognitoId(cognitoId)
        if (!currentUser) {
            throw new Error("User not found")
        }

        // Update user profile in database
        const updatedUser = await this.userMutationService.updateUser(currentUser.id, {
        ...
        try {
            ...
            if (Object.keys(cognitoUpdateParams).length > 0) {
                await this.awsCognitoService.updateUserAttributes(cognitoId, cognitoUpdateParams)
            }
        } catch (cognitoError) {
            // Log error but don't fail the entire operation since DB update succeeded
            this.logger.error(`Failed to update AWS Cognito attributes for user ${cognitoId}:`, cognitoError)
